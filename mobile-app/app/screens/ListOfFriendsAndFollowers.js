...
export default function ListOfFriendsAndFollowers({ navigation, route }) {
    ...
    useEffect(() => {
        ...
        switch (currentTab) {
            case "Friends":
                //API for get friends
                userService.getFriends(user.email)
                .then(res => {
                    setData(res.data);
                })
            ...
            case "Followers":
                ...
                userService.getFollowers(user.email)
                .then(res => {
                    setData(res.data);
                })
                .catch(e => console.error(e))
            ...
            case "Following":
                ...
                userService.getFollowing(user.email)
                .then(res => {
                    setData(res.data);
                })
                .catch(e => console.error(e))
    ...
    return (
        <View style={{ backgroundColor: colors.white }}>
            {showHeader && (<HeaderWithBackArrow
                title={`${user.firstName} ${user.lastName}`}
                onBackButton={() => { navigation.goBack(); }} />)}
            <View style={{marginLeft:15,marginHorizontal:10}}>   
            <Bar tabes={tabes} onTab={handleTabbed} currentTab={currentTab} underLineWidth={currentTab.length * 8} underLineHight={4} activeUnderLineColor={colors.dark} ></Bar>
            </View> 
